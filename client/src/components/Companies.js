    ...
    const fetchCompany = async () => {
      try {
        const response = await axios.post(
          "https://buildforyou.site/api/user/companies",
          {},
          {
            headers: {
...
<nav className="bg-white p-4 flex flex-wrap items-center justify-between gap-20">
                <div className="flex items-center space-x-4">
                    <img src={logo} alt="Logo" className="h-8 w-8 mr-2 md:mr-4" />
                ...
                <div className="mt-2 md:mt-0">
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            className="pl-10 pr-3 py-2 rounded-full border-2 border-gray-300 focus:outline-none focus:border-blue-300 transition duration-300"
  ...
  <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
    {company
      .filter((companys) =>
        search.toLowerCase() === ""
          ? true
          : companys.companyname.toLowerCase().includes(search.toLowerCase())
      )
      .map((companys, index) => (
        <Link to={`/company-details/${companys._id}`} key={index}>
          <div className="cardss bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700 h-full">
            <div className="relative overflow-hidden h-40 bg-cover bg-center">
              <img
                className="h-full w-full object-cover"
                src={`https://buildforyou.site/public/${companys.image}`}
                alt={`Company ${index}`}
              />
              <a href="#!">
            ...
            <div className="p-4">
              ...
              <h2 className="mb-4 text-base text-neutral-600 dark:text-neutral-200">
                Company Name: {companys.companyname}
              </h2>

              <button
                type="button"
                className="project-card-button inline-block rounded bg-primary px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_#3b71ca] transition duration-150 ease-in-out hover:bg-primary-600 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:bg-primary-600 focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:bg_primary-700 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] dark:shadow-[0_4px_9px_-4px_rgba(59,113,202,0.5)] dark:hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)]"
              >
                Company Details
              </button>
            </div>
