  ...
  const handlePayment = async () => {
    try {
      ...
      const orderResponse = await axios.post(
        'https://buildforyou.site/api/user/create-order',
        {
          // Replace with actual user ID
          companyId,
          companyInfo: company.companyname,
          date: date, // Set the selected date
          time: time, // Set the selected time
        },
        {
          headers: {
            Authorization: 'Bearer ' + localStorage.getItem('token'),
  ...
  return (
   ...
   <nav className="bg-white p-4 md:flex md:items-center md:justify-between">
      ...
      <div className="company-header">
        ...
        <h1>Username: {company.companyusername}</h1>
      </div>
